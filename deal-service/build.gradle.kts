...
dependencies {
    ...
    // Hypersistence Utils для Hibernate 6.3+ (PostgreSQL JSONB)
...
foundSpecifications.forEach { specFile ->
    ...
    val basePackage = "com.neoflex.${packageName}"
...
file(".env").takeIf { it.exists() }?.readLines()?.forEach { line ->
    ...
    if (trimmedLine.isNotBlank() && !trimmedLine.startsWith("#") && trimmedLine.contains("=")) {
        val (k, v) = trimmedLine.split("=", limit = 2)
        System.setProperty(k.trim(), v.trim())
        logger.lifecycle("${k.trim()}=${v.trim()}")
    } else {
        logger.debug("Skipping line: $trimmedLine") // для отладки
    }
}
